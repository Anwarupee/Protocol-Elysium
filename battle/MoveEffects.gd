...
static func execute(move: Dictionary, attacker: Monster, defender: Monster, battle_manager: Node) -> void:
	...
	if attacker.firmware_locked:
		var utility_effects = ["defense_buff", "accuracy_buff", "heal", "cleanse_debuff", "super_defense", "pretexting", "type_steal"]
	...
	match move["effect"]:
		...
		"confuse":
			...
			if randf() < 0.4:
				defender.blocked_next_turn = true
				battle_manager.emit_signal("battle_log", "Target confused! May skip next turn!")
		...
		"failover":
			if attacker.hp < attacker.max_hp * 0.25:
				...
				battle_manager.emit_signal("battle_log", "Failover requires HP below 25%!")
		...
		"lure_strike":
			defender.take_damage(int(move["power"] * multiplier))
			battle_manager.emit_signal("battle_log", attacker.monster_name + " lures the target! Honeypot primed!")
